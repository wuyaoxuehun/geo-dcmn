...
def min_pad_batch(input_ids, segment_ids, input_mask):
    flat_input_ids = input_ids.view(-1, input_ids.size(-1))  # [bs*4, sl]
    flat_token_type_ids = segment_ids.view(-1, segment_ids.size(-1))  # [bs*4, sl]
    flat_attention_mask = input_mask.view(-1, input_mask.size(-1))  # [bs*4, sl]

    max_seq_len = torch.max(torch.sum(flat_attention_mask, 1)).item()

    input_ids = flat_input_ids[:, :max_seq_len].view(-1, 4, max_seq_len)  # [bs*4, sl]
    token_type_ids = flat_token_type_ids[:, :max_seq_len].view(-1, 4, max_seq_len)  # [bs*4, sl]
    attention_mask = flat_attention_mask[:, :max_seq_len].view(-1, 4, max_seq_len)  # [bs*4, sl]
    return input_ids, token_type_ids, attention_mask, max_seq_len
...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.local_rank in [-1, 0]:
        ...
        tb_writer = SummaryWriter(flush_secs=10, comment='_LR(%.e)_DS(%s)_SEQLEN(%s)_TOPK(%s)_PNUM(%s)_EPOCH(%s)_model(%s)_(%s)'
                                                         % (args.learning_rate,
                                                            train_files,
                                                            args.max_seq_length,
                                                            args.model_choices,
    ...
    for epoch in train_iterator:
        ...
        import math
        global_log_step = math.floor(len(epoch_iterator) / args.gradient_accumulation_steps) // args.logging_steps
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            try:
                ...
                # input_ids, segment_ids, input_mask, max_seq_len = min_pad_batch(input_ids,
                #                                                                        segment_ids,
                #                                                                        input_mask)
                # if max_seq_len > global_max_seq_len:
                #     global_max_seq_len = max_seq_len

                inputs = {"attention_mask": input_mask, "doc_len": doc_len, "ques_len": ques_len, "option_len": option_len, "labels": label_ids,
                          "token_type_ids": segment_ids, 'input_ids': input_ids}
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 and global_step % global_log_step == 0:
                ...
                logger.info("tr_loss = %s", tr_loss)
                logger.info("logging_loss = %s", logging_loss)
                tb_writer.add_scalar("train_lr", scheduler.get_lr()[0], global_step)
                tb_writer.add_scalar("loss/train_loss", (tr_loss - logging_loss) / global_log_step, global_step)
...
def get_model_predict(args, model, dataset):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        # input_ids, token_type_ids, attention_mask, max_seq_len = min_pad_batch(input_ids,
        #                                                                        segment_ids,
        #                                                                        input_mask)
...
def load_and_cache_examples(args, file, tokenizer, mode):
    utils = MODEL_PROCESS[args.model_type]
    select_field,read_swag_examples,convert_examples_to_features = \
        utils.select_field, utils.read_swag_examples, utils.convert_examples_to_features
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        examples = read_swag_examples(file, args.p_num)
